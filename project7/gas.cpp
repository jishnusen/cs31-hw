...
Flatulan::Flatulan(City *cp, int r, int c) {
  ...
  if (r < 1 || r > cp->rows() || c < 1 || c > cp->cols()) {
    cout << "***** Flatulan created with invalid coordinates (" << r << "," << c
         << ")!" << endl;
    exit(1);
  }
  m_city = cp;
...
int Flatulan::row() const { return m_row; }

int Flatulan::col() const { return m_col; }
...
void Flatulan::move() {
  ...
  int r = m_row;
  int c = m_col;
  m_city->determineNewPosition(r, c,
                               dir); // First cap direction based on map bounds
  if (!m_city->isPlayerAt(r, c)) {   // Only move if player not in location
    m_row = r;
    m_col = c;
  }
}
...
bool Flatulan::possiblyGetConverted() // return true if converted
{
  ...
  return (randInt(1, 3) < 3); // 1,2 out of 1,2,3
}
...
bool Player::isPassedOut() const { return m_health <= 0; }
...
void Player::getGassed() { m_health--; }
...
City::City(int nRows, int nCols) {
  if (nRows <= 0 || nCols <= 0 || nRows > MAXROWS || nCols > MAXCOLS) {
    cout << "***** City created with invalid size " << nRows << " by " << nCols
         << "!" << endl;
    exit(1);
  }
  m_rows = nRows;
...
int City::rows() const { return m_rows; }

int City::cols() const { return m_cols; }

Player *City::player() const { return m_player; }

bool City::isPlayerAt(int r, int c) const {
  return ((m_player->row() == r) && (m_player->col() == c));
}

int City::flatulanCount() const { return m_nFlatulans; }
...
void City::preachToFlatulansAroundPlayer() {
  ...
  for (int i = 0; i < m_nFlatulans; i++) {
    if (m_flatulans[i] != nullptr) {
      auto tmp = m_flatulans[i];
      m_flatulans[i] = m_flatulans[j];
      m_flatulans[j] = tmp;
      j++;
    }
  }
...
void City::moveFlatulans() {
  for (int k = 0; k < m_nFlatulans; k++) {
    ...
    if ((abs(m_flatulans[k]->row() - m_player->row()) == 1) &&
      ...
      m_player->getGassed();
    } else if ((abs(m_flatulans[k]->col() - m_player->col()) == 1) &&
               (m_flatulans[k]->row() == m_player->row())) {
      m_player->getGassed();
    }
  }
}
...
Game::Game(int rows, int cols, int nFlatulans) {
  if (nFlatulans < 0) {
    cout << "***** Cannot create Game with negative number of Flatulans!"
         << endl;
    exit(1);
  }
  ...
  if (rows == 1 && cols == 1 && nFlatulans > 0) {
    cout << "***** Cannot create Game with nowhere to place the Flatulans!"
         << endl;
    exit(1);
  }
...
void doBasicTests() {
  ...
  }
  {
    ...
    for (int r = 1; r <= 2; r++) { // .9999 probability that between 5 and 29
                                   // out of 50 are unconverted
